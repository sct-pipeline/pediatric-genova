...
for metric in dki_FA dki_MD dki_AD dki_RD KFA MK AK RK ; do
  sct_extract_metric -i ${metric}.nii.gz -f segm_gm.nii.gz -vert ${METRICS_VERT_LEVEL} -vertfile /label/template/PAM50_levels.nii.gz -o ${PATH_RESULTS}/${metric}_gm.csv -append 1
  sct_extract_metric -i ${metric}.nii.gz -f segm_wm.nii.gz -vert ${METRICS_VERT_LEVEL} -vertfile /label/template/PAM50_levels.nii.gz -o ${PATH_RESULTS}/${metric}_wm.csv -append 1
  sct_extract_metric -i ${metric}.nii.gz  -f segm_cst.nii.gz -vert ${METRICS_VERT_LEVEL} -vertfile /label/template/PAM50_levels.nii.gz -o ${PATH_RESULTS}/${metric}_cst.csv -append 1
done
cd ..
