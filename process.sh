...
sct_crop_image -i kurtosis.nii.gz -xmin 60 -xmax 100 -ymin 60 -ymax 100 -zmin 8 -o kurtosis_crop.nii.gz
# Motion correction
...
sct_register_to_template -i kurtosis_crop_moco_dwi_mean.nii.gz -s ${file_seg} -ldisc labels_disc1-4.nii.gz -c t1 -ref subject -param step=1,type=seg,algo=centermass:step=2,type=seg,algo=columnwise:step=3,type=im,algo=bsplinesyn,metric=CC,slicewise=1,iter=3 -qc qc
fi
...
sct_maths -i kurtosis_crop_moco_dwi_mean_seg.nii.gz -dilate 3 -o kurtosis_crop_moco_dwi_mean_seg_dil.nii.gz
